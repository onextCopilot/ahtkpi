 ...
 *  - recruitment keys: overview | jobs                       (Tuyển dụng / E-Hiring)
 *  - candidates:       candidates                            (Ứng viên - kho ứng viên, đứng riêng)
 *  - settings keys:    offices | roles | email | channels    (HRM-wide settings)
 */
function hrm_subnav(string $active, bool $isAdmin): void
{
    $settingsKeys = ['offices', 'pipeline', 'owners', 'roles', 'email', 'channels', 'channels_cfg', 'access', 'storage'];
    ...
    // Nhóm Ứng viên (đứng riêng, không thuộc E-Hiring): kho ứng viên + sự kiện.
    $candGroup = [
    ...
    if ($isPlan) {
        ...
        $items = [
            ...
            ['channels_cfg', 'Kênh đăng tin', '/hrm/settings?tab=channels_cfg', '<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>'],
            ['access', 'Phân quyền HRM', '/hrm/settings?tab=access', '<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><line x1="17" y1="11" x2="23" y2="11"/><line x1="20" y1="8" x2="20" y2="14"/>'],
            ['storage', 'Lưu trữ / Ổ cứng', '/hrm/settings?tab=storage', '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14a9 3 0 0 0 18 0V5"/><path d="M3 12a9 3 0 0 0 18 0"/>'],
        ];
    } else {
